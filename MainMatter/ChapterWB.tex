...
\end{tabular}
\end{center}
Which shows a clear discrepency with the exponential model. Note that this
result contradicts that of the previous run, this one should however be more
reliable as the correction factor is smaller (and it's thus less dependent on
the simulation).  
...
\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{figures/Event12397.pdf}
...
Lastly only careful scruteny of the algorithm and code  used throughout this work will tell
if something went wrong with the analysis or something else enterily (e.g the importance of birefringence).

We also redid the fit using only the plane wave method, not making use of snell's law at the surface. Doing
this the only assumption that gets made is that the signal ends up approximately at the same time as in the
simulation ($\pm$ half the period of the signal) but this gave the same value.
...
theoretical models. We'll take a look at this depth with channels 5,6 and 7 but note  
that these will be way less reliable (difficult to quantify how unreliable) as the channels
...
Now that we have some measurements let's plot out all of our findings, the results
are given in figure \ref{fig:IndexVSDepth.pdf}.
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{figures/IndexVSDepth.pdf}
