...
minimizing the difference between the observed and computed time:
\begin{equation}
...
we'll need to specify various functions to minimize.  E.g
if we have four detectors labeled 0 to 3 we'll have to construct
functions between detectors 0\&1, 0\&2, 0\&3, 1\&2, 1\&3 and 2\&3 .  As all of
these correlation functions will have different sizes, but we want to treat
them on equal footing, we'll norm them as follows:
\begin{equation}
	\eta'(\theta) =
	\frac{\eta(\theta)}{\int \eta(\theta)\Delta
	\theta}
\end{equation}
...
time doing a plane wave reconstruction (also using Snell's law at the surface)
and recording the distance from the reconstructed wave to the balloon at the
height of the balloon in an array \textbf{differences}.  Doing this we have a
...
list \textbf{n} is our anwser.

To find the error on this value a first idea was to Gaussian
to this curve of the
form\cite{grabe2005measurement}
\begin{equation}
	g(x) = \frac{1}{\sigma \sqrt{2\pi}} exp\left\{-\frac{1}{2}\frac{(x-\mu)^2}{\sigma^2}\right\}
\end{equation}
but as this fit wasn't
...
\ref{fig:Event12397}. Let's now add the epsilon which we computed
...
This fit shows a clear discrepency with the
...
This is made even more apparent when looking at the index-depth 
relation found in the south polar firn\cite{kravchenko_besson_meyers_2004}:
\begin{equation}
	n(z) = 1.324 - 5.2854z - 21.865z^2 - 39.058z^3
\end{equation}
which would predict an index of 1.66, in stark disagreement with the
exponential model. 
