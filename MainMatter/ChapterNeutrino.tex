...
\begin{equation}
  \mu^- \rightarrow e^- + \bar{\nu}_e + \nu_\mu
\end{equation}
Here the muon decays via a charged current reaction, note that lepton number
needs to be conserved.  This means that, as on the left side of the reaction we
have 1 muon and no electrons we get $\ell_\mu = 1$ and $\ell_e = 0$. This also needs
to be the case on the right side. The lepton number is conserved as a muon
neutrino counts for $\ell_\mu = 1$ and an electron anti-neutrino counts for
$\ell_e = -1$, so on the right we have $\ell_\mu =1$ and $\ell_e = 1-1 = 0$.
