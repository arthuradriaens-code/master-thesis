...
% The title
\thetitle{Development of a new ray tracing algorithm for complex ice models and the analysis of ice properties using weather balloons}
...
\thetitle{\uline{\color{ugentblue}Development of a new ray tracing algorithm for complex ice models 
...
with an initial guess of both being zero. Fitting this function with the
initial guesses you get what's shown on figure \ref{fig:FittedVoltage67}, i.e a
perfect fit.
\begin{figure}
	\begin{minipage}{0.49\textwidth}
...
The difference in timing can be found from using the different $\phi$'s we fitted;
...
\begin{equation}
  V(t) = \sin(2\pi f_c t + \Omega)(1 + m\cos(\omega t + \Phi))
\end{equation}
after some time $t_7$ a signal will reach channel 7 and channel 7 will show a value $V$:
\begin{equation}
  V = \sin(2\pi f_c t_7 + \Omega_7)(1 + m\cos(\omega_7 t_7 + \Phi_7))
\end{equation}
and, after a different (higher) time $t_6$ the same signal will reach channel 6:
\begin{equation}
  V = \sin(2\pi f_c t_6 + \Omega_6)(1 + m\cos(\omega_6 t_6 + \Phi_6))
\end{equation}
Now, for the plane wave reconstruction, we want to know the difference between these arrival times $t_6$ and $t_7$.
\begin{figure}
	\begin{minipage}{0.49\textwidth}
