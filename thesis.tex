...
measure n to be 1.7407 our error will only be 0.0038. After looping through
all the balloon positional files and only outputting the $<5°$ ones we get
some moments balloons were actually close enough (note that $<5°$ almost never happens).

If we search in the DESY
database \footnote{\url{https://rnog-monitor.zeuthen.desy.de/}} within these timeframes
for the particular detectors where the balloon gets close enough to AND the 403MHz signal
coming from the Ballon is detected in the deep channels, the event ... jumps out:
