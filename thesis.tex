...
this thus would mean we'll ask for n at some position:
\begin{mintedbox}{python}
ice = medium.greenland_simple()
n = ice.get_index_of_refraction(position)
\end{mintedbox}
With the position always being inbetween the specific detectors,
e.g if we want to reconstruct the plane wave detected by a detector
at -95 and -97 we'll need to know n at a depth of -96.
Now in reality we don't know the angle a priori, we'll only have the timing
information, so we'll perform a scan by minimizing something called 
...
\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{NormedCorrelation.pdf}
...
where it reaches it's minimum.
\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{SummedCorrelation.pdf}
