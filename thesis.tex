...
There's a little change that needs to happen first however for us to
...
detector.
Now another problem that seemed to arise after removing the 
air observer in the radioproparaytracing.py (where the hybrid ray
tracer is stored) file is that, even though the simulation seems
to work, only the path IN the ice and the timing for that path
is recorded, so the timing was later added by drawing a line
from the balloon to the beginning of the recorded path and 
setting the speed of radio waves in air to c.

Now having established this the first problem we'll consider is
plane wave reconstruction of the original position, an example path
to some of the sensors is given in figure \ref{fig:Example
trajectory}
\begin{figure}
	\centering
...
this thus would mean at position -95.5:
\begin{mintedbox}{python}
position = np.array([0,0,-95.5])
n=medium.greenland_firn.get_index_of_refraction(medium.greenland_firn(),position)
\end{mintedbox}
Now in reality we don't know the angle a priori so we'll perform
a scan by minimizing the correlation function:
\begin{equation}
	Correlation(\theta) = \Delta t 
	- \frac{\cos\theta \Delta z}{v}
\end{equation}
If we have more than 1 station however, than we need to specifiy
various correlation functions.
E.g if we have four detectors labeled 0 to 3 we'll have to construct
correlation functions between detectors 0\&1, 0\&2, 0\&3, 1\&2, 1\&3 and 2\&3 . As all of these correlation functions will have different
...
\begin{equation}
	...
	\theta}
\end{equation}
After this we can sum them and take the minimum of the sum, 
in practice the various correlation functions look like those
shown in figure
