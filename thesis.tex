...
\begin{equation}
	...
	\rho = \rho_0 e^{z/z_0} + B
\end{equation}
And we're done! Another kind of derivation by Schytt was done by assuming the
proportional change in air space to be proportional to the change in pressure
instead:
\begin{equation}
	\frac{dV}{V} \propto d\sigma
\end{equation}
As the volume scales inversly with the density let's assume the relation $V \propto (\rho_i - \rho)$, yielding:
\begin{align}
