...
boundary, it's continous: ice in greenland has a continously varying density.
Because of this we can't work with Snell's law but we'll have to take a look at
the continuous version to figure out how these rays propagate. A path of a ray
$\mathbf{r}(s)$ with path parameter s in a medium with index of refraction
n($\mathbf{r}$) is described by the eikonal equation:
\begin{equation}
	\frac{d}{ds}\left(n(\mathbf{r})\frac{d\mathbf{r}}{ds}\right) = \mathbf{\nabla} n
\end{equation}
This is the "continuous version of Snell's law" which we'll call the
\textit{snell's gradient law}.

The software we'll be using for ray reconstruction is called "radiopropa"
\cite{Winchen_2019} and in radiopropa the local paraxial approximation is used,
i.e if we assume that in any individual step of the algorithm the
change of the refractive index along the path ds is small it's possible to
...
taken and after assuming Schytt's equation to hold (i.e $n \propto \rho$) we find that the
...
index of refraction on depth is called "the exponential model".  

This exponential model has a huge advantage as it's analytically solvable,
meaning that we know instantly which direction we'll have to shoot our ray in
(as mentioned previously) after the location of the neutrino interaction and
...
There is one big downside however: looking at measurements of the density i.f.o
depth as shown in figure \ref{fig:DensityMeasurements} 
\begin{figure}
  \centering
	\includegraphics[width=0.5\textwidth]{Density_measurements2.pdf}
	\caption{Measurements of the density (kg/m$^3$) i.f.o the depth with exponential fit}
...
but would more closely follow some kind of higher order function. this
discrepency between the exponential model and the actual data implies that the
analytic ray tracer will make the wrong predictions.  This is why the
development of a different ray tracer was needed which will be able to handle
more complex ice models, we'll get to this ray tracer in
chapter \ref{chapter:hybrid}.

Lastly there might be one more problem we haven't talked about yet: Birefringe.
We have implicitly assumed that ice is isotropic meaning that both it's
permittivity $\varepsilon$ and it's permeability $\mu$ are scalars but these
could very well be tensorial in nature for radio waves. In general, after
calculating what this tensorial nature implies you'd find that in every
direction two different types of waves can be found each propagating with a
different speed as shown in figure \ref{fig:Fresnel}.
\begin{figure}
  \centering
  ...
  \label{fig:Fresnel}
\end{figure}
Birefringe is the optical property of a material having the \cite{Heyer2023}
