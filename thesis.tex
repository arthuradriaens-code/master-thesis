...
\end{figure}
it seems that the ice actually doesn't follow an exponential
curve as is indicated with the "log function" but more closely
some kind of polynomial. as an example the curve "polynomial
log function" was included which has the structure
\begin{equation}
	a_5 log(x)^5 + a_4 log(x)^4 + a_3 log(x)^3 
	+ a_2 log(x)^2 + a_1 log(x) + a_0
\end{equation}
...
The full code can be found in (WORK) ,
