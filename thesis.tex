...
the second line being the generlaized lorentz force, the third line the yukawa
couplings (rest mass\footnote{nearly all the mass observed in e.g nuclei
actually comes from binding energy.} via higgs boson and massless quark\&lepton field coupling) 
and the last line responsible for the spontaneous symmetry breaking giving rise to 
the rest mass via the Brout Englert Higgs mechanism. We'll mostly concern ourselves with the second part.
\subsection{History}
...
Let's first give a quick example on how parity can be violated in a gauge theory; let's define the operators
\begin{equation}
...
\end{equation}
And the production/annihilation mechanisms of $W/W^\dagger$ bosons from the quark sector:
\begin{equation}
	\mathcal{L}_Q = \sum_j \bar{Q}_j i \slashed{D} Q_j
\end{equation}
We can equate $W$ with $W^-$ and $W^\dagger$ with $W^+$. Here are some diagramattic exaples of possible interactions
which you can conclude from those interaction lagrangian densities\footnote{with u, up-like quarks is meant, e.g charm, top. Analogously with d, down-like quarks}:
\begin{figure}[h!]
...
\begin{figure}[h!]
	...
	\end{minipage}
\end{figure}\\
Combining some of these diagrams we can re-write Fermi's interaction in a renormalizable interaction:
\begin{figure}[h!]
	\centering
	\begin{tikzpicture}
	\begin{feynman}
  \vertex (a0){(u$\mathbf{d}$d)};
	\vertex[above=1cm of a0] (am);
	\vertex[right=2cm of am] (a1);
	\vertex[above=1cm of am] (a2){(u$\mathbf{u}$d)};
	\vertex[right=2cm of a1] (b1);
	\vertex[right=2cm of b1] (bm);
  \vertex[above=1cm of bm] (b2){$e^-$};
  \vertex[below=1cm of bm] (b0){$\bar{\nu}_e$};
	\diagram* {
		{[edges=fermion]
			(a0) -- (a1) -- (a2)
		},
...
end up having moving charges which move faster than the speed of light in the medium,
