...
used for this is called FitN.py and stored at (???), let's go over the full code step
by step:
\subsection{Spatial}
The first part we'll need to concern ourselves with is determining the relative positions
of everything. The balloon data file is and the time when the event took
place are given, from these two the latitudenal and longitudinal position is determined 
and also the elevation. We convert these three measurements to the ENU coordinate frame
and store it in \textit{BalloonPosition}. The next step is to get the location 
The
...
403MHz. As the data is measured in nanoseconds we'll need to convert the frequency
first:
\begin{equation}
	f = 403\text{MHz} = 403\times 10^{-3} \frac{1}{\text{ns}}
\end{equation}
The signal is a sine wave of the form
\begin{equation}
...
\begin{equation}
	\omega = 2\pi f
\end{equation}
So, as $f$ is known, we'll have to fit the amplitude $A$ and the offset $T$
to the data making the differences with the actual signal as close
to zero as possible; if we then compare the different offsets of the sine we
