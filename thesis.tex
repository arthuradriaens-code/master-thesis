...
\begin{figure}[h!]
	...
	\end{minipage}
\end{figure}\\
And with the production of $W/W^\dagger$ bosons from the quark sector:
\begin{equation}
...
\begin{figure}[h!]
	...
	\begin{feynman}
	\vertex (a0){$\bar{d}$};
	\vertex[above=1cm of a0] (ai);
	\vertex[right=1cm of ai] (am);
	\vertex[above=1cm of ai] (a1) {$u$};
	\vertex[right=1cm of am] (af) {$W^+$};
	\diagram* {
		{[edges=fermion]
			(a1) -- (am) -- (a0)
		},
	...
	\vertex[above=1cm of a0] (ai);
	\vertex[right=1cm of ai] (am);
	\vertex[above=1cm of ai] (a1) {$\bar{u}$};
	\vertex[right=1cm of am] (af) {$W^-$};
	\diagram* {
		{[edges=fermion]
			(a0) -- (am) -- (a1)
	...
	\end{minipage}
\end{figure}\\
And various other interactions, notice that 
we've equated $W$ with $W^-$ and $W^\dagger$ with $W^+$.
%\subsection{Majorana}
