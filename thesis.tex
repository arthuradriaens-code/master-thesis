...
Deze extreem hoge energy neutrino telescoop zal astrofysische neutrinos zoeken
...
achtergrondinformatie, daarna wordt in hoofdstuk 5 het bouwen van een algoritme
uitgelegd en in hoofdstuk 6 worden indirecte metingen gedaan gebruik makende
...
doel het snel vinden van een pad vanuit een radio bron vertex (bv neutrino
...
tau neutrinos. But for them to oscillate into eachother, theoretically they not
only require mass but each flavor also should have a different mass as the 2D
...
Looking back at our energy per nucleon vs number of nucleons in the nucleus figure shown in the
...
Fission reactors operate by splitting this urantium via the introduction of a neutron:
\begin{equation}
...
Cherenkov radiation.  Cherenkov radiation is like the elektromagnetic
equivalent of a sonic boom, a sonic boom happens when something goes faster
than the sounds speed in the medium; A particle emits Cherenkov radiation if it
...
\end{figure}
Waves propagating through ice to a detector generally looks something like
...
(TBC)).
This exponential model has a huge advantage however as it's analytically 
...
Now how much data do we have that way? After looping through every weather
...
we get the result shown in figure \ref{fig:EpsilonIFODirectSnell}\footnote{The code for this can be found in projects-mt/BaLLooN/simulations as plane\_wave\_with\_snell.py}
\begin{figure}
	\centering
...
Running ahead on the actual measurements, let's estimate how accurate our predictions will be,
for the moment we're only interested in the deep channels 0-3. Let's label the vertical spatial accuracy, i.e how
accurately we know the height of these detectors, $\delta z$ and the timely accuracy $\delta t$.
Even though we know our $\epsilon$ to always overestimate the actual value, meaning that we could
theoretically account for this by just calculating $\epsilon$ for the considered system and 
correcting, let's assume it to be an absolute error. If we want to fit n, we first minimize the 
summed correlation function to construct the plane wave. One correlation function is given by 
\begin{equation}
	Correlation(\theta) := \Delta t - \Delta t' = \Delta t
	- \frac{\cos\theta \Delta z}{v}
\end{equation}
...
\begin{align}
  \delta n &= \sqrt{\left(\frac{\partial n}{\partial \Delta t}\right)^2\delta t^2 + \left(\frac{\partial n}{\partial \Delta z}\right)^2\delta z^2}\\
      &= \sqrt{\left(\frac{\partial n}{\partial \Delta t}\right)^2\delta t^2 + \left(\frac{\partial n}{\partial \Delta z}\right)^2\delta z^2}\\
      &= \frac{c}{\cos{\theta_{min}}}\sqrt{\left[\frac{1}{\Delta z} - \frac{\mathcal{C}}{\Delta z}\right]^2\delta t^2 + 
      \left[\frac{\mathcal{C}}{\Delta z^2} - \frac{\Delta t}{\Delta z^2}\right]\delta z^2}
\end{align}
As we actually summed the correlation functions (6 in total), it's equivalent to doing 6 independent measurements
of n, meaning that we get a reduction by a factor $\sim \sqrt{6}$.
Now due to the inherent inaccuracy as previously shown, this n also has an additional error of $\pm \epsilon(\vec{r})$
with $\vec{r}$ the position of the balloon. Our final error on n is thus:
\begin{equation}
  \pm \epsilon(\vec{r})\pm\frac{c}{\sqrt{6}\cos{\theta_{min}}}\sqrt{\left[\frac{1}{\Delta z} - \frac{\mathcal{C}}{\Delta z}\right]^2\delta t^2 + 
      \left[\frac{\mathcal{C}}{\Delta z^2} - \frac{\Delta t}{\Delta z^2}\right]\delta z^2}
\end{equation}
\section{Fitting the index}
Now that we know our goal to be feasible, let's analyse some data. The
positional data of the weather balloons was obtained from the
\url{ftp1.esrl.noaa.gov} website using the rno-g-sonde script Of the official
RNO-G github page. We'll be looking at the data recorded over the summer of
2022, more particularly 15/06/2022 - 30/09/2022. The detected events can be
obtained from the DESY
database\footnote{\url{https://rnog-monitor.zeuthen.desy.de/}}.
...
(station 21) and subsequently very close to Terianniaq (station 12) as is 
also found in appendix \ref{app:10Deg}.  If we take a look at this day in the event
...
from these particular detectors. The time window we'll need to look at is
inbetween 11:24:02 and 11:26:23. This seems to be within
...
a frequency of 403 as shown in figure \ref{fig:BalloonFreq}. Now taking a
look at event 1981 in run 528 which was recorded at 20/08/2022 11:25:51 we see
the spectrum shown in figure \ref{fig:DetectorFreq} which shows a clear spike
at 403MHz for the surface detectors.
\begin{figure}
  \centering
