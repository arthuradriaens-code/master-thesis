...
\end{tabular}
\end{center}
As can be seen all of the fitted indices show a discrepancy with the index from
the exponential model. Out of these measurements the last one that's listed (the one
based on station 11) is the most reliable as it has a small $\epsilon$ and uses
the closely spaced phased array. But this fit also shows the largest discrepency to
the exponential model, it can be argued that this shows a shortcoming in the currently
...
Uit deze 5 metingen is de laatste de meest betrouwbare, deze heeft de kleinste correctiefactor $\epsilon$ en
gebruikt de dich bij elkaar gelegen channels van de phased array.
Hoewel dit het geval is toont deze het grootste verschil met het exponentieel model, wat toont
dat er mogelijk iets scheelt aan dat model.
